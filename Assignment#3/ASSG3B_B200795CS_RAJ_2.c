...
int isVisited(int* arr, int* len, int key){
    int ll = *len;
    for(int i=0; i<ll; i++)
        if(arr[i]==key) 
            return 1;
...
int dfs(struct node* root, int* arr, int* len, int x){
    if(!root) return 0;
    if(!x){
        if(root->color) return 1;
        else return 0;
    }

    if(isVisited(arr,len,root->val)) return 0;
    else{
        arr[(*len)++]=root->val;
        return dfs(root->p,arr,len,x-1)||dfs(root->left,arr,len,x-1)||dfs(root->right,arr,len,x-1);
    }
}

int BeautifulNode(struct node* root, int x){
    if(!root) return 0;

    // To check if the current node is beautiful or not.
    int* arr = (int*)malloc(500*sizeof(int)); int len=0;
    int val=dfs(root,arr,&len,x);
    free(arr);
    
    return val+BeautifulNode(root->left,x)+BeautifulNode(root->right,x);
}
